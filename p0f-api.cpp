...
extern fp_configurator* my_fp_configurator;

/* Process API queries. */

void handle_query(struct p0f_api_query* q, struct p0f_api_response* r, fp_configurator *my_fp_configurator) {
  ...
  switch (q->addr_type) {
    ...
    case P0F_ADDR_IPV4:
    case P0F_ADDR_IPV6:
      h = PROCESSOR.lookup_host(q->addr, q->addr_type);
      break;
...
#pragma GCC diagnostic ignored "-Wstringop-truncation"
  if (h->last_name_id != -1) {
    ...
    strncpy((char*)r->os_name, (char*)my_fp_configurator->fp_os_names[h->last_name_id],
            P0F_STR_MAX + 1);
  ...
  if (h->http_name_id != -1) {
    ...
    strncpy((char*)r->http_name, (char*)my_fp_configurator->fp_os_names[h->http_name_id],
            P0F_STR_MAX + 1);
