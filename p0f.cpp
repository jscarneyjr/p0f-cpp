...
static void open_api(void) {
  ...
  api_cl = (api_client *)DFL_ck_alloc(api_max_conn * sizeof(struct api_client));
  ...
  do {
    ...
    if (a) {
      ...
      if (a->addr->sa_family == PF_INET)
        SAYF("     IP address  : %s\n", utils::addr_to_str(((u8*)a->addr) + 4, IP_VER4));
      else
        SAYF("     IP address  : %s\n", utils::addr_to_str(((u8*)a->addr) + 8, IP_VER6));
...
retry_no_vlan:
  ...
  if (!orig_rule) {
    ...
    if (vlan_support) {
      ...
      final_rule = (u8 *)ck_alloc(strlen((char*)orig_rule) * 2 + 64);
      ...
      final_rule = (u8 *)ck_alloc(strlen((char*)orig_rule) + 16);
  ...
  if (pcap_compile(pt, &flt, (char*)final_rule, 1, 0)) {
    ...
    // should use const char * but does not
    // dangerous
	pcap_perror(pt, (char *)"[-] pcap_compile");
...
static void live_event_loop(void) {
  ...
  pfds = (pollfd *)ck_alloc((1 + (api_sock ? (1 + api_max_conn) : 0)) *
                  sizeof(struct pollfd));

  ctable = (api_client **)ck_alloc((1 + (api_sock ? (1 + api_max_conn) : 0)) *
...
poll_again:
    ...
    for (cur = 0; cur < pfd_count; cur++) {
      ...
      if (pfds[cur].revents & POLLIN) switch (cur) {
        ...
        case 0:
          ...
          if (pcap_dispatch(pt, -1, (pcap_handler)processor::static_parse_packet, reinterpret_cast<u_char *>(&PROCESSOR)) < 0)
            FATAL("Packet capture interface is down.");
        ...
        default:
          ...
          if (ctable[cur]->in_off == sizeof(struct p0f_api_query)) {
            ...
            handle_query((struct p0f_api_query *)(&ctable[cur]->in_data),
            		(struct p0f_api_response *)(&ctable[cur]->out_data),my_fp_configurator);
  ...
  ck_free((VOID_PTR)ctable);
  ck_free((VOID_PTR)pfds);
