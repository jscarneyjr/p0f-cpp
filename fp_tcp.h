...
class fp_tcp {
public:
	fp_tcp(processor *a_processor,fp_configurator *a_fp_configurator);
	...
	// private:
	u8 guess_dist(u8 ttl);
	s16 detect_win_multi(struct tcp_sig* ts, u8* use_mtu, u16 syn_mss);
	void tcp_find_match(u8 to_srv, struct tcp_sig* ts, u8 dupe_det,  u16 syn_mss);
	void packet_to_sig(struct packet_data* pk, struct tcp_sig* ts);
	u8* dump_sig(struct packet_data* pk, struct tcp_sig* ts, u16 syn_mss);
	u8* dump_flags(struct packet_data* pk, struct tcp_sig* ts);
	void score_nat(u8 to_srv, struct tcp_sig* sig, struct packet_flow* f);

	struct tcp_sig_record* sigs[2][SIG_BUCKETS];
	u32 sig_cnt[2][SIG_BUCKETS];
private:
	// globals
