...
public:
	...
	// for pcap callbacks
	static void static_parse_packet(u_char *user, const pcap_pkthdr *hdr, const u_char *bytes);
...
private:
    ...
    u32 get_host_bucket(u8* addr, u8 ip_ver);
    void destroy_host(struct host_data* h);
    void nuke_hosts(void);
    struct host_data* create_host(u8* addr, u8 ip_ver);
    void touch_host(struct host_data* h);
    void destroy_flow(struct packet_flow* f);
    void nuke_flows(u8 silent);
    struct packet_flow* create_flow_from_syn(struct packet_data* pk);
 ...
 private:
	...
	struct host_data *host_by_age,   /* All host entries, by last mod      */
	                        *newest_host;   /* Tail of the list                   */

	struct packet_flow *flow_by_age, /* All flows, by creation time        */
