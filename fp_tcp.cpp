...
fp_tcp::fp_tcp(processor *a_processor, fp_configurator *a_fp_configurator){
...
s16 fp_tcp::detect_win_multi(struct tcp_sig* ts, u8* use_mtu, u16 syn_mss) {
...
void fp_tcp::tcp_find_match(u8 to_srv, struct tcp_sig* ts, u8 dupe_det,
                           u16 syn_mss) {
...
void fp_tcp::tcp_register_sig(u8 to_srv, u8 generic, s32 sig_class, u32 sig_name,
  ...
  tsig = (tcp_sig *)DFL_ck_alloc(sizeof(struct tcp_sig));
  ...
  sigs[to_srv][bucket] = (tcp_sig_record *)DFL_ck_realloc((VOID_PTR)(sigs[to_srv][bucket]),
...
/* Convert struct packet_data to a simplified struct tcp_sig representation
 ...
 void fp_tcp::packet_to_sig(struct packet_data* pk, struct tcp_sig* ts) {
  ...
  ts->ts1         = pk->ts1;
  ts->recv_ms     = my_processor->get_unix_time_ms();
  ts->matched     = NULL;
  ts->fuzzy       = 0;
  ts->dist        = 0;
...
u8* fp_tcp::dump_sig(struct packet_data* pk, struct tcp_sig* ts, u16 syn_mss) {
...
#define RETF(_par...) do { \
    s32 _len = snprintf(NULL, 0, (const char *)_par); \
...
u8* fp_tcp::dump_flags(struct packet_data* pk, struct tcp_sig* ts) {
...
void fp_tcp::score_nat(u8 to_srv, struct tcp_sig* sig, struct packet_flow* f) {
...
struct tcp_sig* fp_tcp::fingerprint_tcp(u8 to_srv, struct packet_data* pk,
                                struct packet_flow* f) {

  struct tcp_sig* sig;
  struct tcp_sig_record* m;

  sig = (tcp_sig *)ck_alloc(sizeof(struct tcp_sig));
...
void fp_tcp::check_ts_tcp(u8 to_srv, struct packet_data* pk, struct packet_flow* f) {
  ...
  if (to_srv) {
     ...
     ms_diff = my_processor->get_unix_time_ms() - f->client->last_syn->recv_ms;
     ...
     ms_diff = my_processor->get_unix_time_ms() - f->server->last_synack->recv_ms;
