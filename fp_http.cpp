...
fp_http::fp_http(processor *a_processor, fp_configurator *a_fp_configurator){
	my_processor = a_processor;
	my_fp_configurator = a_fp_configurator;

	hdr_names = NULL;              /* List of header names by ID         */
	hdr_cnt = 0;                   /* Number of headers registered       */

	for(int i = 0; i < SIG_BUCKETS;i++){
	   hdr_by_hash[i] = NULL;;  /* Hashed header names                */
	   hbh_cnt[i] = 0;          /* Number of headers in bucket        */
	}
	for(int i=0;i<2;i++){
	  sigs[i]=NULL;
	  sig_cnt[i]=0;
    }
	ua_map=NULL;   /* Mappings between U-A and OS        */
	ua_map_cnt=0;
...
s32 fp_http::lookup_hdr(u8* name, u32 len, u8 create) {
  ...
  hdr_names = (u8 **)DFL_ck_realloc((VOID_PTR)hdr_names, (hdr_cnt + 1) * sizeof(u8*));
  hdr_names[hdr_cnt] = DFL_ck_memdup_str((VOID_PTR)name, len);

  hdr_by_hash[bucket] = (u32 *)DFL_ck_realloc((VOID_PTR)hdr_by_hash[bucket],
    (hbh_cnt[bucket] + 1) * 4);
...
void fp_http::http_find_match(u8 to_srv, struct http_sig* ts, u8 dupe_det) {
  ...
  while (cnt--) {
    ...
    for (rs_hdr = 0; rs_hdr < rs->miss_cnt; rs_hdr++) {
      ...
      for (ts_hdr = 0; ts_hdr < ts->hdr_cnt; ts_hdr++)
        if (rs->miss[rs_hdr] == (u32)ts->hdr[ts_hdr].id) goto next_sig;
...
void fp_http::http_register_sig(u8 to_srv, u8 generic, s32 sig_class, u32 sig_name,
  ...
  hsig = (http_sig *)DFL_ck_alloc(sizeof(struct http_sig));

  sigs[to_srv] = (http_sig_record *)DFL_ck_realloc((VOID_PTR)(sigs[to_srv]), sizeof(struct http_sig_record) *
    (sig_cnt[to_srv] + 1));
...
void fp_http::http_parse_ua(u8* val, u32 line_no) {
  ...
  while (*val) {
    ...
    id = my_fp_configurator->lookup_name_id(val, nxt - val);
    ...
    ua_map = (ua_map_record *)DFL_ck_realloc((VOID_PTR)ua_map, (ua_map_cnt + 1) *
                        sizeof(struct ua_map_record));

    ua_map[ua_map_cnt].id = id;
   
    if (!name) ua_map[ua_map_cnt].name = my_fp_configurator->fp_os_names[id];
...
u8* fp_http::dump_sig(u8 to_srv, struct http_sig* hsig) {
...
#define RETF(_par...) do { \
    ...
    ret = (u8 *)DFL_ck_realloc_kb(ret, rlen + _len + 1); \
  ...
  for (i = 0; i < hsig->hdr_cnt; i++) {
    ...
    if (hsig->hdr[i].id >= 0) {
      ...
      while (list->name) {
        if ((s32)list->id == hsig->hdr[i].id) break;
        list++;
      }
  ...
  while (list->name) {
    ...
    for (i = 0; i < hsig->hdr_cnt; i++) 
      if (hsig->hdr[i].id == (s32)list->id) break;
...
u8* fp_http::dump_flags(struct http_sig* hsig, struct http_sig_record* m) {
...
void fp_http::score_nat(u8 to_srv, struct packet_flow* f) {
  ...
  if (m->class_id == -1) {
    ...
    my_processor->verify_tool_class(to_srv, f, m->sys, m->sys_cnt);
    ...
    if (ref && ref->matched) {
      ...
      } else if (ref->matched->name_id != (s32)m->label_id) {
  ...
  if (to_srv && m->class_id == -1 && f->http_tmp.sw && !f->http_tmp.dishonest) {
    ...
    if (i != ua_map_cnt) {
      ...
      if ((s32)ua_map[i].id != hd->last_name_id) {
...
header_check:
  ...
  my_processor->add_nat_score(to_srv, f, reason, score);
...
void fp_http::fingerprint_http(u8 to_srv, struct packet_flow* f) {
  ...
  if (!to_srv) {
    ...
    ck_free((VOID_PTR)f->server->http_resp);
    f->server->http_resp = (http_sig *)ck_memdup((VOID_PTR)(&f->http_tmp), sizeof(struct http_sig));
    ...
    if (m) {
      ...
      if (m->class_id != -1) {
        ...
        ck_free((VOID_PTR)f->client->http_req_os);
        f->client->http_req_os = (http_sig *)ck_memdup((VOID_PTR)(&f->http_tmp),
...
void fp_http::free_sig_hdrs(struct http_sig* h) {
...
u8 fp_http::parse_pairs(u8 to_srv, struct packet_flow* f, u8 can_get_more) {
  ...
  while ((off = f->http_pos) < plen) {
    ...
    if (pay[off] == '\r' || pay[off] == '\n') {
      ...
      f->http_tmp.recv_date = my_processor->get_unix_time();
...
u8 fp_http::process_http(u8 to_srv, struct packet_flow* f) {
